...
```
which will also install `RmdConcord` and
`backports` from Github. `RmdConcord` provides
the R Markdown concordance support; `backports`
supplies some functions from R
which are not available in pre-4.3.0
versions.  Finally, the latest changes to
`patchDVI` have not yet made
it to CRAN.
