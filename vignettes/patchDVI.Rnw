...
\abstract{
The \patchDVI package works with Sweave \citep{Leisch02},  knitr \citep{Xie2013} and R Markdown \citep{Xie2022}
...
for \file{.pdf}
output, first in \file{pdfsync}.  More recently
Synctex \citep{Laurens08} links have been
implemented in \texttt{pdflatex} and other \LaTeX\ processors.

Unfortunately for knitr/Sweave/R Markdown users, these links point to the \file{.tex}
source that was processed, which is not the true source code in the
knitr/Sweave \file{.Rnw} or R Markdown \file{.Rmd} or other input file.  (I will
...
\verb!patchDVI::pdf_documentC!.  The driver
...
There are several ways to make use of \patchDVIn.  This section
describes some common ones for Sweave/knitr.  Section \ref{sec:markdown}
...
With \file{.Rnw} files for knitr or Sweave, the
...
\section{R Markdown Instructions}\label{sec:markdown}

The most recent addition to \patchDVI is support for R Markdown
input.  R Markdown \citep{Allaire2022} has become very popular in
recent years, with \file{.Rmd} input nowadays exceeding the
popularity of Sweave-style
\file{.Rnw} input.  The general structure is that each file contains a ``YAML
...
remotes::install_github("dmurdoch/patchDVI")
@
The \textbf{backports} package supplies some functions from R
which are not available in pre-4.3.0 versions.  The \textbf{knitr package}
...
With those versions installed, you then change the \verb!output!
setting in the YAML.  If it was \verb!pdf_document!, use\\
\verb!patchDVI::pdf_documentC! \\
instead.  If it
was \verb!html_document!, use\\
\verb!patchDVI::html_documentC!.  \\
That should be
...
\texttt{SweavePDF}/\texttt{knitPDF} family of
functions; the ones intended for Markdown source are
\texttt{renderMiktex}, \texttt{renderPDFMiktex}, \texttt{renderDVI},
\texttt{renderPDF}, \texttt{renderDVIPDFM} and \texttt{renderAll}.

\subsection{Known limitations of Markdown support}

The Markdown support relies on a relatively recent addition to
Pandoc, the \verb!sourcepos! extension.  At present (Pandoc 2.18
...
\patchDVI switches the Pandoc input setting to \verb!commonmark_x+sourcepos!.

Unfortunately, the \verb!commonmark_x! reader doesn't support most of
the extensions that the standard \verb!markdown! reader supports.
This page:  \url{https://github.com/jgm/pandoc/wiki/Roadmap} lists the current status.
For the definitions of the extensions, see the Pandoc documentation
at \url{https://pandoc.org/MANUAL.html}.)

Of particular note are the lack of \verb!citations! and
\verb!raw_tex!.  Citations in Markdown source like \verb!(@Author)!
...
\subsection{Technical description of concordance records}\label{sec:records}
...
project management functions, \texttt{knitAll}, \texttt{renderAll} and
\texttt{SweaveAll}.  These are really the same function with different
...
any of \label{fulllist} \Sexpr{paste0("\\\\texttt{", outer(c("knit", "Sweave", "render"), c("PDF", "Miktex", "DVI", "DVIPDFM"), paste0), "\175", collapse=", ")}
...
The functions
are all named in the same style:  \textit{Stage1Stage2}, where
\textit{Stage1} is one of \texttt{knit}, \texttt{render}, or \texttt{Sweave}, and \textit{Stage2} is one of \texttt{DVI}, 
\texttt{Miktex}, \texttt{PDF}, \texttt{PDFMiktex}, \texttt{DVIPDFM}.
The full list was given near the end of the introduction to Section \ref{sec:make} on p. \pageref{fulllist}.  The \textit{Stage1} parts
specify the initial processor as being \texttt{knit::knit}, \texttt{Sweave}, or \texttt{rmarkdown::render}.  The \textit{Stage2}
part specifies how the resulting \texttt{.tex} files are processed:
\texttt{knitDVI} and \texttt{knitMiktex} and related functions produce \texttt{.dvi} files 
in the general case and for MikTeX respectively; \texttt{knitPDF} and
\texttt{knitPDFMiktex} and friends do the same for direct \file{.pdf} output
from \texttt{pdflatex}.  Finally, \texttt{knitDVIPDFM} and 
...
locale line can be omitted if you only use plain ASCII characters, but is probably necessary
for other cases.
\item To use with R Markdown, create a file called \\
\verb!Library/TeXShop/Engines/markdown.engine! \\
containing the lines
\begin{verbatim}
#!/bin/bash
export LC_ALL=<locale>
...
\item When editing a \texttt{.Rnw} file in TeXShop, choose the knitr or Sweave engine from the menu; for \texttt{.Rmd} files, use the markdown
engine.
...
In step \ref{theCommand}, replace the second line of the command (the \verb!SweavePDF! call) with the following:
\begin{verbatim}
patchDVI::renderPDF('$fullname', envir = globalenv())
\end{verbatim}
