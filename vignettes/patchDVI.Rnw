...
(For the definitions of these extensions, see the Pandoc documentation
at \url{https://pandoc.org/MANUAL.html}.)
Of the extensions that are enabled by default, \verb!commonmark_x!
...
table_captions
\end{verbatim}
Of the extensions that are disabled by default, these are not supported:
\begin{verbatim}
...
will not be processed in Commonmark.  Raw \LaTeX\ like
\verb!\pagebreak! will need to be ``fenced'', i.e. entered as
\begin{verbatim}
```{=latex}
\pagebreak
```
\end{verbatim}
I don't know if there's a way to avoid a paragraph break around
fenced \LaTeX.

There are also two extensions available in \verb!commonmark_x! that
are not available in Pandoc Markdown.  The \verb!sourcepos! extension
is the most important one; it's the whole basis of concordance
support.  Additionally, the \verb!attributes! extension
(enabled by default) subsumes several of the standard \verb!*_attributes!
extensions.  The \verb!gfm_auto_identifiers! extension is enabled
in \verb!commonmark_x! but disabled in Pandoc Markdown.
