...
for Japanese language text when using Sweave.  I expect the issues would be the same
in other non-Roman alphabets, but I have no experience with
them.  If you are using \texttt{.Rnw} files in
\texttt{knitr}, see the vignette ``Using knitr and \patchDVI with Japanese text''.
